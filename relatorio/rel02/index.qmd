...
[R] é uma linguagem de programação multi-paradigma com foco em análise de dados e estatística computacional, é uma linguagem de código aberto e possui uma grande comunidade ativa, que contribui para o desenvolvimento de recursos e pacotes, o que torna [R] uma linguagem amplamente adotada por diversas áreas, como estatística e ciência de dados.
...
Neste relatório, será apresentado como realizar a configuração inicial para programar em [R], além de apresentar os conceitos básicos da linguagem e como ela pode ser útil para análises estatísticas e manipulação de dados.
...
- Instale a versão apropriada ao seu sistema operacional do [R].
- Instale o [RStudio].
...
Nesta parte será apresentada a sintaxe básica da linguagem [R], incluindo a estrutura de comandos, a forma de escrever código e as convenções de nomenclatura. A sintaxe é a base para a escrita de qualquer linguagem de programação, é fundamental a compressão da sintaxe para a construção de códigos em [R].
...
::: {.callout-note}
Veja as seções de [operadores básicos](#sec-operadores), [tipos de dados](#sec-tipos-dados) e [funções básicas](#sec-funcoes) para entender melhor a sintaxe da linguagem [R] e o código acima.
:::
...
#### Tipos de dados básicos: {#subsec-tipos-dados-basicos}

- Numéricos: para armazenar números, como 1, 2.5, etc.

  ::: {.callout-note}
  Em [R] todos os números são armazenados como numéricos, mesmo que sejam inteiros, floats ou de outros tipos, a linguagem lida por trás dos panos com a conversão de tipos, então não é necessário se preocupar com isso.
  :::
...
- Vetores: para armazenar uma sequência de elementos do mesmo tipo(homogêneos).
...
- Listas: para armazenar uma coleção de elementos de diferentes tipos(heterogêneos).
...
- Data Frames: para armazenar dados heterogêneos em uma estrutura bidimensional.
...
#### Operadores aritméticos: `+`, `-`, `*`, `/`, `^` {#subsec-operadores-aritmeticos}

Esses operadores são usados para realizar operações matemáticas básicas, como adição, subtração, multiplicação, divisão e exponenciação. Por exemplo:
...
Esses operadores são usados para combinar condições lógicas. O operador `&` representa a conjunção (AND), o operador `|` representa a disjunção (OR) e o operador `!` representa a negação (NOT). Por exemplo:
